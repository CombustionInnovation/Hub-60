<?php
	...
	//	public $folder = "/home/dan/public_html/hub/uploads";
		...
		function uploadImage($user_id) {
			...
			if (is_uploaded_file($_FILES['filename']['tmp_name'])){
				...
				//	$user->insertImage("http://combustioninnovation.com/luis/pinstant/uploads/".$_FILES ['filename'] ['name'],$ip);
					...
					$location = "http://combustionlaboratory.com/hub/pictures/".$_FILES ['filename'] ['name'].$user_id;
					$filename = $_FILES ['filename'] ['name'].$user_id;
					$output = $location;
					list($width_orig, $height_orig) = getimagesize("../pictures/".$filename);
					if($width_orig > 650)
					{
						$this -> resSaveImage("../pictures/".$filename,630,"");
					}
    			}
				else {
		...
		function watermarkImage($pic) {
			...
			$im = imagecreatefromjpeg($pic);
			imagestring($stamp, 5, 20, 20, 'Pinstant', 0x0000FF);
