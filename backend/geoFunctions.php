<?
	class geoFunction{
		...
		$location['city'] = $output->results[0]->address_components[2]->long_name;//using google geocode api we get the city name and we then add them to the pin
		$location['state'] = $output->results[0]->address_components[4]->long_name;//using google geocode api we get the state name and we then add them to the pi
		return $location;
			}
