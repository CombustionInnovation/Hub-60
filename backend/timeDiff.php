<?php
	...
	function getMinuteDiff($mydate){
		$date = new DateTime(date('Y-m-d H:i:s',time()));
		//Retrieve passed times, convert to Y-m-d g.i a
		$datetwo = new DateTime($mydate);
		$from_time = strtotime($datetwo->format('Y-m-d H:i:s')); 
		$to_time = strtotime($date->format('Y-m-d H:i:s')); 
		//Calculate difference between two dates
		$mindiff =  ($to_time - $from_time) ;
	
		//If difference < 1, both dates occurring now (simultaneously)
		
		return abs(floor($mindiff));
